...
(function (root, factory) {
  ...
  var tpl = [
    ...
    '<h2 class="cal-backcolor"><i class="icon iconfont icon-fon10" id="prevMonth"></i>{%date%}<i class="icon iconfont icon-Previous" id="nextMonth"></i></h2>',
        ...
        if (roleName == 'PATIENT') {
          if (visitToDoList != '' && visitToDoList != null && visitToDoList != undefined) {
            ...
            for (var v = 0; v < visitToDoList.length; v++) {
              ...
              if (util.formatDate(new Date()) == util.formatDate(new Date(visitToDoDate))) {
                $("#patientTitle").html(util.formatDate(new Date()) + ',祝您安康</br>按照医嘱，本日您可以提交随访资料');
                $("#patientVisit").show();
              } else if (util.formatDate(new Date()) >= util.formatDate(new Date(visitCanCreateBegin))) {
                $("#patientTitle").html(util.formatDate(new Date()) + ',祝您安康</br>按照医嘱，本日您可以提交随访资料');
                $("#patientVisit").show();
              } else if (util.formatDate(new Date()) <= util.formatDate(new Date(visitCanCreateEnd))) {
                $("#patientTitle").html(util.formatDate(new Date()) + ',祝您安康</br>按照医嘱，本日您可以提交随访资料');
                $("#patientVisit").show();
              } else {
                $("#patientTitle").html(util.formatDate(new Date()) + ',祝您安康</br>本日没有医嘱待办事项');
                $("#patientVisit").hide();
              }

              var classStyle = '';
              if (item.date == util.formatDate(new Date(visitToDoDate))) {
                classStyle = 'crf1_class section_class';
                foundPatient = true;
                break;
              } else {
                if (item.date == util.formatDate(new Date(visitCanCreateBegin))) {
                  classStyle = 'createBegin_class';
                  foundPatient = true;
                  break;
                } else if (item.date == util.formatDate(new Date(visitCanCreateEnd))) {
                  classStyle = 'createEnd_class';
                  foundPatient = true;
                  break;
                } else if (item.date > util.formatDate(new Date(visitCanCreateBegin)) && item.date < util.formatDate(new Date(visitCanCreateEnd))) {
          ...
          if (toDoDateList != '' && toDoDateList != null && toDoDateList != undefined) {
            ...
            for (var k = 0; k < toDoDateList.length; k++) {
              ...
              if (item.date == util.formatDate(new Date(toDoDate))) {
                found = true
                break;
              }
            }
