...
function modifyPatientData() {
    ...
    var paramValue = data + 'siteId=' + sessionStorage.getItem('siteId') + '&roleId=' + sessionStorage.getItem('roleId') +
        '&userId=' + sessionStorage.getItem('userId') + "&visitId=-1&patientId=" + patientId + "&currentModuleDefineId=&isCompleted=" + isCompleted;

    mui.ajax(path + 'crf/modifyPatientData_interface.do?' + paramValue + paramsAcc, {
        data: {},
        dataType: 'json',//服务器返回json格式数据
        type: 'post',//HTTP请求类型
        success: function (data) {
