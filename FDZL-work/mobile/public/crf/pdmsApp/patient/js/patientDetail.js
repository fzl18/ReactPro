...
// 				id: '/DopaSpace',
// 			});
// 		} else if (roleFlag == 'MyPanel') {
// 			mui.openWindow({
// 				url: '/MyPanel',
// 				id: '/MyPanel',
// 			});
// 		}
// 		// else if(roleFlag == 'Calendar'){
// 		// 	mui.openWindow({
...
// 		if (roleFlag == 'Chat') {
// 			mui.openWindow({
// 				url: sessionStorage.getItem('chatUrl'),
// 				id: sessionStorage.getItem('chatUrl')
// 			});
// 		} else if (roleFlag == 'Calendar') {
// 			mui.openWindow({
// 				url:'../../../calendar/index.html',
	...
	mui.ajax(path + 'patient/getInvestigationPatientBaseInfo.do?' + _params + paramsAcc, {
		...
		success: function (data) {
			if (data.error) {
				...
				var patientNameType = sessionStorage.getItem('patientNameType');
				if (patientNameType == 1) {
					patientNameCode = data.patientNameCN;
				} else {
...
function getRoleController() {
	...
	contrlo_key_param += "arrays[0]=onlyViewCrf_btn&";
	contrlo_key_param += "arrays[1]=viewCrf_btn&";
	contrlo_key_param += "arrays[2]=commitVisit_btn&";
	contrlo_key_param += "arrays[3]=deleteVisit_btn&";
	contrlo_key_param += "arrays[4]=turnDownVisit_btn&";
			...
			success: function (data) {
				if (data.error) {
					...
					for (var i = 0; i < _list.length; i++) {
						...
						if ("onlyViewCrf_btn" == keyName && keyValue) {
							haveLook = true;
						} else if ("viewCrf_btn" == keyName && keyValue) {
							haveRecord = true;
						}
					}

					if (haveLook && haveRecord) {
						isCRC = true;
					} else if (haveLook) {
						isCRA = true;
					}
