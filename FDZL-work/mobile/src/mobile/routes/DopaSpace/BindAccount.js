...
class BindAccount extends React.Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener('popstate', () => {  // popstate监听返回按钮
      // window.WeixinJSBridge.call('closeWindow');    // 执行
  ...
  onChange = (e) => {
    if (e.nativeEvent.value == '患者') {
      this.setState({
        role: 'patient',
        selectedIndex: 0,
      });
    } else if (e.nativeEvent.value == '医生') {
      this.setState({
        role: 'doctor',
        selectedIndex: 1,
      });
  ...
  sendIdentify = () => {
    ...
    this.props.dispatch({ type: 'BindAccount/verifyCode', payload: { _captcha_, username } });
    ...
    return (
      ...
      <div style={{ height: '100%', width: '100%', position: 'absolute', maxWidth: 800 }} >
        ...
        <div className={styles.Accountbotton} >
          ...
          >
            <p style={{ color: 'white', fontSize: 16 }}>绑 <span style={{ marginLeft: '15%' }}> 定</span></p></Button>
