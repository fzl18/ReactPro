...
class Chat extends React.Component {
  ...
  render() {
    ...
    const { role, acctId } = sessionStorage;
    const { doctorCompellation, doctorPosition,
      patientCompellation, patientHospitalizationNumber } = this.props.info;
      ...
      <div className={styles.chatBox}>
         ...
         (<div className={styles.PatientBox} >
           <div className={styles.heading}>
             ...
             <div className={styles.Patientinfo}>
               <p style={{ fontSize: 16, color: 'white', marginLeft: 30 }}>{doctorCompellation} <span>{doctorPosition}</span></p>
          ...
          (<div className={styles.DocBox} >
            <div className={styles.heading}>
              ...
              <div className={styles.docinfo}>
                <p style={{ fontSize: 14, color: 'rgb(220,220,220)', marginLeft: 0 }}>提问者: <span onClick={this.getPatientDetail} style={{ color: 'rgb(81,243,196)' }}>{patientCompellation}</span><span onClick={this.getPatientDetail} style={{ color: 'rgb(81,243,196)' }}> ({patientHospitalizationNumber})</span></p>
                <WingBlank size="sm" ><hr className={styles.hr} /></WingBlank>
                <p style={{ fontSize: 14, color: 'rgb(220,220,220)', marginTop: 15, marginLeft: 0 }}>咨询对象: <span>{doctorCompellation}</span><span style={{ marginLeft: 5 }}>{doctorPosition}</span></p>
        ...
        (<div className={styles.PatientBox} >
          <div className={styles.heading}>
            ...
            <div className={styles.Patientinfo}>
              <p style={{ fontSize: 16, color: 'white', marginLeft: 45 }}>{userCompellation} <span>{hotDoctorPosition}</span></p>
              <p style={{ fontSize: 13, color: 'white', marginTop: 15, marginLeft: 45 }}>擅长: <span className={styles.adept}>{doctorAdept}</span></p>
            </div>
          </div>
        </div>)
             : null}
        ...
        <PullToRefresh
          ref={el => this.ptr = el}
          style={{
            height: this.state.height - 110 - (this.props.id == 'hotChat' ? 0 : 53),
            overflow: 'auto',
          }}
        ...
        >
          <div className="chat_scroll" style={{ padding: '0px 5px' }}>
            ...
            data.map((val, valIndex) => {
              if ((val.ydataAcctId == acctId && val.source === 'USER') || (this.props.id == 'hotChat' && val.accType == 'PATIENT')) { // 我的消息及热门咨询医生的消息
                return (
                  <div key={`${val.time}${valIndex}`} className={`${styles.clear} ${styles.chat_msg}`}>
                    <div className={`${styles.me} ${styles.message}`}>
                      <div className={styles.avatar}>
                        {
                          this.props.id == 'hotChat' ?
                            <i className="icon iconfont icon-ren" />
                          :
                            <span>
                              {
              ...
              } else if ((val.ydataAcctId != acctId && val.source === 'USER') || (this.props.id == 'hotChat' && val.accType == 'DOCTOR')) { // 别人的消息及热门咨询医生信息
                return (
                  <div key={`${val.time}${valIndex}`} className={`${styles.clear} ${styles.chat_msg}`}>
                                      ...
                                      <p key={msgListIndex}>
                                        {msgList.map((robotMsg, roIndex) => {
                                          if (robotMsg.type == 'Common') {
                                            return (
                                              <span key={`${robotMsg.text}${roIndex}`} style={{ color: robotMsg.color }}>
                                                {robotMsg.text}</span>
                                            );
                ...
                return (
                  <div key={`${val.time}${valIndex}`} className={styles.msg_system}>
                    {
                      val.showTime &&
                      <div className={`${styles.time}`}>
                        {val.showTime}
