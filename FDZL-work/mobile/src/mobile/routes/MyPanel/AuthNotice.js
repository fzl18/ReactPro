          ...
          <ElementAuth auth="notice">
            ...
            >
              <Item
                ...
                extra={obj.createTime && moment(obj.createTime).format('YYYY-MM-DD')}
                align="top"
              >
                <i
                  className="iconfont icon-shouquanzh-"
                  style={{ color: '#62cf96', fontSize: 24, marginRight: '10px', position: 'relative' }}
                >
                  {
                  obj.status == 'UNREAD' ?
                    <span style={{ position: 'absolute', borderRadius: '6px', border: '5px solid #f00' }} />
                    :
                    null
                }
                </i>授权通知<Brief>{obj.noticeInfo}</Brief>
              </Item>
            </SwipeAction>
          </ElementAuth>
        </List>
